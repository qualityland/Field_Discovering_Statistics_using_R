...
library(foreign)
lecturerData <- read.spss("Lecturer Data.sav",
                          use.value.labels = TRUE,
                          to.data.frame = TRUE)
lecturerData$birth_date <- as.Date(as.POSIXct(lecturerData$birth_date , origin =
                                                "1582-10-14"))


#--------Exporting files-----------
write.table(metallica,
            "Metallica Data.txt",
            sep = "\t",
            row.names = FALSE)
...
alcoholPersonality <- subset(lecturerData,
                             alcohol > 10,
...
soberPeople <- subset(lecturerData,
                      alcohol <= 12,
                      select = c("name", "job", "income", "friends"))

neuroticOrAlcoholic <- lecturerData[alcohol >= 20 | neurotic > 14, ]
neuroticOrAlcoholic <- subset(lecturerData, alcohol >= 20 |
                                neurotic > 14)

#--------Restructuring Data-----------
satisfactionData = read.delim("Honeymoon Period.dat", header = TRUE)
satisfactionStacked <- stack(
  satisfactionData,
  select = c(
    "Satisfaction_Base",
...
Method <- factor(Method,
                 levels = c(1:2),
                 labels = c("Electric Shock", "Being Nice"))
